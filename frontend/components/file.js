...
export default {
  view(vnode) {
    const file = vnode.attrs.file;
    const idx = vnode.attrs.idx;
    return div.file.card(
      div.section(
      ...
      UserService.loggedIn()
        ? div.section(
            ...
            a.button(
              {
                onclick: () => {
                  MapService.reveal();
                  LogService.info(
                    "Please select a point on the map by clicking"
                  );
            ...
            a.button({ onclick: () => (expand = !expand) }, expand ? "^" : "v"),
          )
        : null,
      expand ? div.section(pre(JSON.stringify(file, undefined, 2))) : null,
